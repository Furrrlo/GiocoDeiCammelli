...
public class PoolList {
    ...
    private static final int SCREEN_BORDER = 100;
    /** @brief Bordo da uitilizzare tra due vasche adiacenti */
    private static final int POOLS_BORDER_HEIGHT = 10;
    ...
    private final List<PoolEntity> list;
     ...
     *         dimensioni
     */
    public PoolList(RenderManager renderManager,
    ...
    private void addAll() {
        ...
        final float lenghtX = (res.getScaledWidth() - (SCREEN_BORDER * 2)) / columns;
        final float lenghtY = (res.getScaledHeight() - (SCREEN_BORDER * 2)) / rows;

        float lastX = SCREEN_BORDER;
        float lastY = SCREEN_BORDER;

        for (int i = 0; i < rows; i++) {

            float topBorder = i != 0 ? POOLS_BORDER_HEIGHT : Float.MAX_VALUE; // If it's the first row, upper border is infinite
            float bottomBorder = i + 1 != rows ? POOLS_BORDER_HEIGHT : Float.MAX_VALUE; // If it's the last row, bottom border is infinite

            for (int j = 0; j < columns; j++) {

                float leftBorder = j != 0 ? POOLS_BORDER_HEIGHT : Float.MAX_VALUE; // If it's the first column, left border is infinite
                float rightBorder = j + 1 != columns ? POOLS_BORDER_HEIGHT : Float.MAX_VALUE; // If it's the last column, right border is infinite
